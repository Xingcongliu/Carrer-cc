...
// Fourth cloud
    clouds.push_back(make_unique<Circle>(white, 600, 200, 20));
    clouds.push_back(make_unique<Circle>(white, 630, 200, 20));
    clouds.push_back(make_unique<Circle>(white, 620, 190, 20));
    clouds.push_back(make_unique<Rect>(white, 615, 205, cloudBottom));

    // Fifth cloud
    clouds.push_back(make_unique<Circle>(white, 200, 180, 20));
    clouds.push_back(make_unique<Circle>(white, 230, 180, 20));
    clouds.push_back(make_unique<Circle>(white, 220, 170, 20));
    clouds.push_back(make_unique<Rect>(white, 215, 185, cloudBottom));

    // Sixth cloud
    clouds.push_back(make_unique<Circle>(white, 500, 300, 20));
    clouds.push_back(make_unique<Circle>(white, 530, 300, 20));
    clouds.push_back(make_unique<Circle>(white, 520, 290, 20));
    clouds.push_back(make_unique<Rect>(white, 515, 305, cloudBottom));

    // Seventh cloud
    clouds.push_back(make_unique<Circle>(white, 100, 300, 20));
    clouds.push_back(make_unique<Circle>(white, 130, 300, 20));
    clouds.push_back(make_unique<Circle>(white, 120, 290, 20));
    clouds.push_back(make_unique<Rect>(white, 115, 305, cloudBottom));
...
//    }
    ...
    // Check if the user is overlapping with the grass
    if (grass.isOverlapping(user)) {
        glutDestroyWindow(wd);
        exit(0);
    }

    for (int i = 0; i < clouds.size(); i += 4) {

        if (dynamic_cast<Circle&>(*clouds[i]).isOverlapping(user)||  dynamic_cast<Circle&>(*clouds[i+1]).isOverlapping(user)||  dynamic_cast<Circle&>(*clouds[i+2]).isOverlapping(user)){
            glutDestroyWindow(wd);
            exit(0);
        }
...
void message(string label)
{
    glColor3f( 0, 0, 0 );
    glRasterPos2f(0, height/5);
    int len, i;
...
void message2(string label)
{
    glColor3f( 0, 0, 0 );
    glRasterPos2f(0, height/5+10);
    int len, i;
...
void startScreen()
{
    ...
    message2("Using mouse to move and avoid the cloud and grass");
